...
func convertSentinelInfoOutput(
	...
	for scanner.Scan() {
		...
		if strings.HasPrefix(name, "master") {
			//master0:name=circles_redis,status=ok,address=10.104.0.38:6379,slaves=2,sentinels=3
			master_status := strings.Split(parts[1], ",")
			if len(master_status) > 4 {
				if strings.Split(master_status[1], "=")[1] == "ok" {
					fields["master_status"] = 1
				} else {
					fields["master_status"] = 0
				}
				fields["master_slaves"], _ = strconv.ParseInt(strings.Split(master_status[3], "=")[1], 10, 64)
				fields["master_sentinels"], _ = strconv.ParseInt(strings.Split(master_status[4], "=")[1], 10, 64)
			}
			continue
		}
